...
To test the validity of the proposed SRAM cell, a set of BSIM3 PTM models were used in HSPICE to simulate the cells. Within the program, the authors created four different CMOS cells using four different lengths: 180nm, 90nm, 65nm, 45nm. They kept the $V_{DD}$ constant at $0.9V$ and used butterfly curves for each of the three SNM variations. For the each of the SNM, they found that the HSNM improved by 24.85%, the RSNM improved by 7.72%, and the WSNM improved by 5.94%. They found that the reduced parasitic capacitance and stronger access transistors overpowering the pull-up PMOS ($M_1$ and $M_3$) helped both the read and write actions in improving their noise margins. They also found that delay improved with the 45nm offering only 12ps of delay for each cell, which is a 14% improvement over the state-of-art. However, natural $V_{th}$ variability and an increase leakage current are downsides in their design. Figure 13 below shows the experimental results for the HSNM, RSNM, and WSNM at the 45nm process:
...
To accomplish this, the author focuses on two core techniques for noise reduction: **auto-zeroing (AZ)** and **chopper stabilization (CHS)**. As the name states, the auto-zeroing method samples and cancels noise by storing errors in capacitors during a “zeroing phase.” This allows for both DC offset and noise to be temporarily “eaten” and then released since the capacitors can discharge back into the system. However, the solution does cause more overhead in both area and power while also creating more thermal noise in the system. Chopper stabilization (CHS), on the other hand, utilizes the frequency domain to reduce noise overall. It will take an incoming signal and modulate it to a higher frequency away from the noise in the frequency domain. It will then amplify only the clean signal and then demodulate it back to the original frequency. This makes it so that the new signal now greatly over-powers the 
...
[7] N. Zheng and P. Mazumder, “[[Modeling and Mitigation of Static Noise Margin Variation in Subthreshold SRAM Cells]],” _IEEE Transactions on Circuits and Systems I: Regular Papers_, vol. 64, no. 10, pp. 2726–2736, Oct. 2017. doi:10.1109/tcsi.2017.2700818
[8] C. A. Dos Reis, “[[A Review of Offset and Noise Reduction Techniques for CMOS Amplifiers]],” _Journal of Integrated Circuits and Systems_, vol. 17, no. 1, pp. 1–9, Apr. 2022. doi:10.29292/jics.v17i1.572
