Date: 3rd September 2025
Date Modified: 3rd September 2025
File Folder: Week 2
#environmental

```ad-abstract
title: Today's Topics
collapse: open

- 

```

# UN 2030 Agenda for Sustainable Development

There are *17* goals that focus on:
1. Human concerns
2. Environmental issues

![[Pasted image 20250903102842.png | center]]

# Greenland Vikings 2: Electric Boogaloo

```ad-question
The big question is "why was the settlement unsustainable?"
```

The *five* factors:
1. Natural climate change (for ice cores)
2. Self-Inflicted Environmental damage (from mud cores)
	- Overgrazing
	- Soil-erosion
	- Overharvesting of grass (used as insulation)
	- Harvested a lot of timber as well
3. Failure to respond to the environment (From Garbage Heaps)
	- Lack of fish in their diet, even though they lived on the coast
	- Switched to some marine resources (walrus and seals)
4. Hostile Neighbors
	- Did not adopt Inuit customs
5. Loss of Friendly Neighbors
	- European suppliers stopped arriving

## What is An Epoch

A unit of geologic time marked by distinct characteristics in rock layers

![[Pasted image 20250903102358.png | center]]

```ad-summary
title: The Antropocenic Era
Since the industrial revolution, scientists believe we are entering a new epoch called the **anthropocene**. 
- Named after the root word "caused by human actions."
- This was also sped up by the *Great Acceleration* post WWII.
```

What has been added to the crust?
- Plastics
- Radioactive materials
- Metal scraps
- Pesticides

```ad-important
The UN found through their Millennium Ecosystem Assessment that our current ways are unsustainable. 
```

